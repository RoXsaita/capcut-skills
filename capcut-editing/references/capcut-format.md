...
Reverse-engineered and version-specific. This is a schema reference, not a manual write
procedure. Use `capcutctl` for project mutations; its transactions preserve document identity,
snapshot changes, validate and roll back on failure.
## Paths
...
The CLI clones compatible donor entries with fresh ids when creating segments. Use `add`
or the appropriate high-level operation; inspect `src/add.mjs` in the CLI when extending it.
## Keyframes (punch-in / pan)

```python
{"id":UUID, "material_id":"", "property_type":"KFTypeScaleX",
 "keyframe_list":[{"id":UUID,"curveType":"Line","time_offset":SOURCE_START_US,
...
## Validation and safe project creation

`capcutctl doctor --project NAME` checks material references, timing and document identity.
Preserve the empty main video track: it is part of the overlays-only project structure.
An error-free doctor report does not establish speech continuity or picture quality.

```bash
capcutctl new --project NAME --blank
capcutctl new --project NAME --from TEMPLATE
```

`new` creates the draft metadata and registry entry. CapCut must be fully closed for project
writes; closing only the editor window is insufficient. Do not copy folders, remove lock
files or update `root_meta_info.json` by hand.

## Root draft, active timeline and mirrors

A project can contain these files:
...
The root draft and active timeline are **separate documents**. Apply the same operation with
consistent generated ids to both, then synchronize each document's own mirrors. They can have
different top-level data; do not require byte equality between the two document groups.
Inactive timelines are independent edits and must not be overwritten with the active timeline.

`capcutctl` owns that pairing and the timeline id links. `doctor` checks it; `sync` repairs
mirror drift. Never glob every timeline directory and write one JSON blob to all of them.

Duplicated drafts can share timeline UUIDs. Blank projects receive new ids; a cloned project
must retain internally consistent ids, folder names and `main_timeline_id`. Let `new` manage
this rather than treating observed ids as globally fixed or manually regenerating one field.
...
**Preset 3**, parks its template clips 30 seconds after the new scenes, registers the draft
...
geometry from `presets/layouts.json` (captured from a verified reference timeline). Do not
...
The white ring carries a circle mask of its own. Earlier hand-built rings lacked it
and sat about 5px off their avatars; re-running `layout circle` supplies the correct mask.
...
Higher track index renders in front. `track_render_index` mirrors the track position;
keep `render_index` monotonic with that order so legacy readers agree. Do not reorder
visible layers solely from inherited `render_index` values.

## The layer stack — measured from `reference B`
...
top row.** Track order is the model used by the CLI compositor.
...
All **9** of reference B's transitions sit on track 4, the talking head. None sit anywhere
...
2. **A clip on both sides.** A transition on a segment with nothing after it on its own
   ...
   wrong the moment it opens. This cost a payoff cue in `reviewed test edit` — 15 written,
   14 survived — and was invisible until the transitions were counted after a re-save.

Reference B achieves (2) by slicing the face at every visible cut **even where the face content
...
```bash
capcutctl qa --project NAME --times 1.5,6,41.5 --guide 960 --out qa/
```
